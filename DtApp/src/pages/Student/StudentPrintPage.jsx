            ...
            <form onSubmit={handleSubmit} className={styles.form}>
                ...
                <div className={styles.formGroup} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #555', padding: '1rem', borderRadius: '4px' }}>
                    ...
                    <div className={styles.toggleGroup}>
                        <button type="button" onClick={() => setColor('B&W')} className={color === 'B&W' ? styles.toggleActive : ''}>B&W</button>
                        <button type="button" onClick={() => setColor('Color')} className={color === 'Color' ? styles.toggleActive : ''}>Color</button>
                    </div>
                </div>
                ...
                <div className={styles.formGroup} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #555', padding: '1rem', borderRadius: '4px' }}>
                    ...
                    <div className={styles.toggleGroup}>
                        <button type="button" onClick={() => setIsStapled(false)} className={!isStapled ? styles.toggleActive : ''}>No Stapling</button>
                        <button type="button" onClick={() => setIsStapled(true)} className={isStapled ? styles.toggleActive : ''}>Staple</button>
                    </div>
                </div>
            ...
            <div className={styles.jobList}>
                ...
                {jobs.map(job => (
                    <div key={job.id} className={styles.jobCard}>
                        <div><strong>Slot: {job.slotId}</strong> - {job.files ? `${job.files.length} file(s)` : 'Legacy Job'}</div>
                        <span className={styles.statusBadge} style={{ backgroundColor: getStatusColor(job.status) }}>{job.status}</span>
                    </div>
                ))}
            </div>
        </div>
